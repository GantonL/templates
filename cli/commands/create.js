...
export default async function createCommand(projectName, options) {
  try {
    ...
    if (!availableTemplates.includes(templateName)) {
      throw new CLIError(`Template "${templateName}" not found. Available: ${availableTemplates.join(', ')}`);
    }
    ...
    // Copy template
    const copySpinner = logger.spinner(`${icons.gear} Copying template files...`);
    copySpinner.start();
    ...
    if (!options.skipGit) {
      const gitSpinner = logger.spinner(`${icons.gear} Initializing git repository...`);
      gitSpinner.start();
    ...
    if (!options.skipInstall) {
      ...
      const installSpinner = logger.spinner(`${icons.gear} Installing dependencies with ${packageManager}...`);
      ...
      try {
        ...
        logger.warning(`Please run ${formatCommand(`cd ${targetDir} && ${packageManager} install`)} manually`);
      }
    }
    ...
    const nextSteps = [
      ...
      ...(options.skipInstall ? [`${formatCommand(`${templateInfo.packageManager || 'npm'} install`)}`] : []),
