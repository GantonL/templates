...
export function setCookie(event: RequestEvent, cookieCategory: CookieCategory['name'], cookie: {[key: string]: string}) {
...
export function cookieSetRequest(cookie: {[key: string]: string}) {
  ...
  return fetch('/manage-cookies', {
    method: 'POST',
    body,
 })
}

export function getCookie(event: RequestEvent, cookieCategory: CookieCategory['name'], cookieName: string): string | undefined {
  const currentPreferences = getUserCookiesPreferences(event);
  if (!currentPreferences[cookieCategory]) {
    return;
  }
